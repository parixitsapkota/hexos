...
u32 get_fat_entry_lba(FAT32_BPB *fat32, u32 cluster, u32 partition_lba) {
  u32 fat_start_lba = partition_lba + fat32->BPB_RsvdSecCnt;
...
u8 fat32_find_file_in_dir(Disk disk, FAT32_BPB *fat32, u32 partition_lba, u32 dir_cluster,
  ...
  u32 bytes_per_cluster = fat32->BPB_SecPerClus * fat32->BPB_BytsPerSec;

  u8 *dir_buffer = falloc(bytes_per_cluster);
  if (!dir_buffer) {
    return 0;
  }
  ...
  u32 current_dir_cluster = dir_cluster;

  while (current_dir_cluster < 0x0FFFFFF8 && current_dir_cluster > 0) {

    u32 dir_lba = cluster_to_lba(fat32, current_dir_cluster, partition_lba);
    lba_read(disk, dir_lba, fat32->BPB_SecPerClus, dir_buffer);

    u32 max_entries = bytes_per_cluster / sizeof(FAT_DirEntry);
    ...
    for (u32 i = 0; i < max_entries; i++) {
      ...
      if (fmemcmp(entries[i].DIR_Name, dos_name, 11) == 0) {
        *out_start_cluster = ((u32)entries[i].DIR_FstClusHI << 16) | entries[i].DIR_FstClusLO;
    ...
    u32 fat_lba = get_fat_entry_lba(fat32, current_dir_cluster, partition_lba);
    u32 fat_offset = get_fat_entry_offset(fat32, current_dir_cluster);

    u8 *fat_sector = falloc(fat32->BPB_BytsPerSec);
    if (!fat_sector) {
      ffree(dir_buffer);
      return 0;
    }
    ...
    current_dir_cluster = (*(u32 *)(fat_sector + fat_offset)) & 0x0FFFFFFF;
    ffree(fat_sector);
  }
...
u8 *fat32_read_file(Disk disk, FAT32_BPB *fat32, u32 partition_lba, u32 start_cluster,
  ...
  u8 *file_buffer = falloc(file_size);
  if (!file_buffer) {
    return NULL;
  }

  u32 cluster_bytes = fat32->BPB_SecPerClus * fat32->BPB_BytsPerSec;
  u8 *cluster_buffer = falloc(cluster_bytes);
  u8 *fat_sector_buffer = falloc(fat32->BPB_BytsPerSec);
  ...
  while (current_cluster < 0x0FFFFFF8) {
    ...
    u32 cluster_lba = cluster_to_lba(fat32, current_cluster, partition_lba);
    lba_read(disk, cluster_lba, fat32->BPB_SecPerClus, cluster_buffer);

    u32 bytes_to_copy = (bytes_remaining < cluster_bytes) ? bytes_remaining : cluster_bytes;
    ...
    u32 fat_lba = get_fat_entry_lba(fat32, current_cluster, partition_lba);
    u32 fat_offset = get_fat_entry_offset(fat32, current_cluster);
